...
This artifact contains the code, data, and expected CSV outputs for the experiments reported in the submitted ACSAC paper. It is organized according to the ACSAC Artifact Evaluation layout:
...
> **Note:** The `artifact/data/` directory is not included in this repository due to file size constraints (total ~2.1 GB).
>
> If you need access to the evaluation data, please contact the authors by email and we will share it with you directly.

The data directory contains:
- `extended-features/` — monthly Android feature JSON files
- `AndroZoo-Year/` — yearly AndroZoo-derived CSV
- `features/` — DREBIN and Marvin feature pickles used for cluster-level evidence inspection
...
- `common.py`
- `validation_hyperparameter_tuning.py`
- `baseline_static.py`, `baseline_full_update.py`
- `proposed_performance_suite.py`
- `selection_control_ablation.py`
- `yearly_detection_performance.py`, `yearly_ablation.py`
- `drebin_evidence_inspection.py`, `marvin_evidence_inspection.py`
- `cluster_review_behavior.py`
- `data_distribution_export.py`, `baseline_table_export.py`
- `paired_ttest_analysis.py` — regenerates paper-style paired t-test and 95% CI CSVs for Tables VI–VII from archived period-level F1 logs

`artifact/results/full_csv/` contains archived full CSV logs (same relative paths as a full rerun under `./results/`).  
...
**3. Regenerate paired t-test CSVs (Tables VI–VII):**
```bash
...
The artifact provides monthly and yearly hyperparameter JSON files under `artifact/results/full_csv/hyperparameters/`. Downstream experiments load those JSON files instead of retuning on test data. Shared representation/protocol/reporting settings such as D=2^18, d=64, K₀=20, T_ttl=3, ρ_t, TopK, and classifier α are fixed before validation; validation selection is limited to the four paper-tuned Proposed parameters. `validation_hyperparameter_tuning.py` is included for optional audit/regeneration, but the default full rerun uses the provided JSON to avoid spending evaluator time on parameter search.
